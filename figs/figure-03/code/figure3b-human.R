...
cat("\n--- Single test (one-sided paired t-test: SIRVs > TUSCO) for TP category ---\n")
df_tp <- all_data %>% filter(big_category == "TP")
wide_tp <- df_tp %>%
  select(pipeline, Type, percentage) %>%
  pivot_wider(names_from = Type, values_from = percentage)

if ("TUSCO" %in% colnames(wide_tp) && "SIRVs" %in% colnames(wide_tp) &&
  sum(!is.na(wide_tp$TUSCO)) > 1 && sum(!is.na(wide_tp$SIRVs)) > 1) {
  # One-sided paired t-test: SIRVs > TUSCO (equivalent to TUSCO < SIRVs)
  test_out_rev <- t.test(wide_tp$TUSCO, wide_tp$SIRVs, paired = TRUE, alternative = "less")
  pval_rev <- test_out_rev$p.value
  cat("Category: TP\n")
  cat("p-value (SIRVs > TUSCO) = ", pval_rev, "\n")
} else {
  cat("Not enough data for TP category to perform SIRVs > TUSCO test.\n")
}
...
# This section extracts key values for manuscript updates
extraction_file <- file.path("..", "..", "..", "manuscript", "extraction_values.tsv")

# Get p-values for PTP and FN (indices 2 and 4 in categories vector)
ptp_pval <- p_values[2] # PTP p-value
fn_pval <- p_values[4] # FN p-value

# Prepare extraction data
extraction_data <- data.frame(
  figure_id = "fig-3b",
  metric_name = c("human_PTP_pvalue", "human_FN_pvalue"),
  value = c(formatC(ptp_pval, format = "e", digits = 3), formatC(fn_pval, format = "e", digits = 3)),
  ci_lower = NA,
  ci_upper = NA,
  notes = "One-sided paired t-test (TUSCO > SIRVs)",
  stringsAsFactors = FALSE
)

# Write or append to file
if (!file.exists(extraction_file)) {
  write.table(extraction_data, extraction_file,
    sep = "\t",
...
cat(
  "Extracted p-values: PTP =", formatC(ptp_pval, format = "e", digits = 3),
  ", FN =", formatC(fn_pval, format = "e", digits = 3), "\n"
)
# === END EXTRACTION ===
