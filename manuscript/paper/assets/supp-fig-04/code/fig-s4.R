...
p <- ggplot(df, aes(x = platform_method, y = FN_pct)) +
  ...
  # Add significance annotations manually if ggsignif is not available
  {
    if (has_ggsignif) {
      geom_signif(
        comparisons = list(
          c("PacBio-cDNA", "ONT-cDNA"),
          c("PacBio-cDNA", "ONT-dRNA")
        ),
        test = "wilcox.test",
      ...
      annotate("text", x = 2, y = 50, label = paste0("p(PacBio vs ONT) < 0.05"),
  ...
  labs(
    ...
    subtitle = paste0("Kruskal-Wallis p = ", format(kw_p, digits = 2, scientific = TRUE))
  ) +
