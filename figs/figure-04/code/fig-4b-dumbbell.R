...
make_metric_label_plot <- function() {
  ggplot() +
    ...
    theme(
      ...
      axis.text.y = element_text(size = 6.5, hjust = 1, margin = margin(r = 1.5, unit = "mm")),
...
# ------------------------------------------------------------------
# Build legends (shape = annotation; color = species), matching Fig 4B style
# ------------------------------------------------------------------
legend_ann_df <- data.frame(
  Annotation = factor(c("Gencode reference annotation", "TUSCO-novel"),
                      levels = c("Gencode reference annotation", "TUSCO-novel")),
  x = c(1, 2), y = c(1, 2)
)
p_legend_ann <- ggplot(legend_ann_df, aes(x = x, y = y, shape = Annotation)) +
  geom_point(size = 3, color = "black") +
  scale_shape_manual(values = c("Gencode reference annotation" = 21, "TUSCO-novel" = 24)) +
  theme_void() +
  theme(
...
legend_annotation <- cowplot::get_legend(p_legend_ann)
...
p_legend_species <- ggplot(legend_species_df, aes(x = x, y = y, color = Species)) +
  ...
  scale_color_manual(values = c("Human" = species_colors[["human"]], "Mouse" = species_colors[["mouse"]])) +
  theme_void() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
legend_combined <- plot_grid(legend_annotation, legend_species, ncol = 2, rel_widths = c(2, 1))
...
for (i in seq_along(pipeline_row_specs)) {
  ...
  if (row_label == "Iso-Seq + SQ3 ML") row_label <- "Iso-Seq\n+\nSQ3 ML"
  row_label_grob <- ggdraw() +
    draw_label(row_label, fontfamily = "Helvetica", fontface = "bold", size = 7, angle = 0, hjust = 1) +
...
overall_grid_with_legend <- plot_grid(overall_grid_padded, legend_combined, ncol = 1, rel_heights = c(1, 0.16))
