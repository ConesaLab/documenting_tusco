...
run_figure3b <- function(species = c("human", "mouse"), write_files = TRUE, test_method = c("perm", "wilcox", "t")) {
  ...
  # Map test_method to human-readable name for extraction notes
  test_method_name <- switch(
    test_method,
    "perm"   = "sign-flip permutation test",
    "wilcox" = "Wilcoxon signed-rank test",
    "t"      = "paired t-test",
    test_method  # fallback to raw value
  )
  ...
  extraction_data <- data.frame(
    ...
    ci_lower = NA,
    ci_upper = NA,
    notes = paste0("Two-sided paired test (", test_method_name, "; TUSCO vs SIRVs); direction from estimated shift (TUSCO-SIRVs)"),
