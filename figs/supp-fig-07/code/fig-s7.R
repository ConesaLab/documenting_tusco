...
###############################################################
# Figure S7 - SIRV metrics across replicate combinations
# Usage: Rscript fig-s7.R [output_dir] [width] [height]
###############################################################
...
out_pdf <- file.path(plot_dir, "fig-s7.pdf")
...
points_s7_path <- file.path(plot_dir, "fig-s7_points.tsv")
bars_s7_path   <- file.path(plot_dir, "fig-s7_bars.tsv")
points_s7_tsv  <- file.path(tsv_dir,  "fig-s7_points.tsv")
bars_s7_tsv    <- file.path(tsv_dir,  "fig-s7_bars.tsv")

if (!local_only) {
  all_points_sirv_df <- collect_points_for_sirv()
  if (!all(c("Tissue","Samples","Combo","Metric","Value") %in% names(all_points_sirv_df))) {
    warning("No points found or missing columns; proceeding with empty dataset for fig-s7.")
  ...
  readr::write_tsv(all_points_sirv_df, points_s7_path)
  readr::write_tsv(summary_sirv_df, bars_s7_path)
  readr::write_tsv(all_points_sirv_df, points_s7_tsv)
  readr::write_tsv(summary_sirv_df, bars_s7_tsv)
} else {
  if (!file.exists(points_s7_path) || !file.exists(bars_s7_path)) {
    stop("LOCAL_ONLY is set, but required SIRV TSVs are missing in ", plot_dir, ". Expected: ",
         basename(points_s7_path), ", ", basename(bars_s7_path))
  }
  all_points_sirv_df <- readr::read_tsv(points_s7_path, show_col_types = FALSE) %>%
    mutate(
      Tissue = factor(Tissue, levels = c("Brain", "Kidney")),
      Metric = factor(Metric, levels = metric_order_sirv),
      SamplesLabel = as.character(Samples)
    )
  summary_sirv_df <- readr::read_tsv(bars_s7_path, show_col_types = FALSE) %>%
...
plots_s7 <- lapply(metric_levels_sirv, function(m) create_metric_barplot_all(m, all_points_sirv_df, summary_sirv_df, "#cab2d6"))
fig_s7_panel <- plot_grid(plotlist = plots_s7, ncol = 3, nrow = 2, align = "hv")

sirv_underlying_out <- file.path(tsv_dir, "fig-s7.tsv")
sirv_all_points_out <- all_points_sirv_df %>%
  arrange(Tissue, Samples, Combo, Metric) %>%
  mutate(figure_id = "fig-s7", panel_id = "s7_points")
sirv_bar_means_out <- summary_sirv_df %>%
  ...
  mutate(figure_id = "fig-s7", panel_id = "s7_bars")
sirv_export_df <- bind_rows(
  sirv_all_points_out %>% mutate(dataset = "s7_points"),
  sirv_bar_means_out %>% mutate(dataset = "s7_bars")
)
